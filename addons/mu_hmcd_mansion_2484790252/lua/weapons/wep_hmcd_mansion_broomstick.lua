...
SWEP.ViewModel = "models/mu_hmcd_mansion/weapon_poker/v_poker.mdl"
SWEP.WorldModel = "models/mu_hmcd_mansion/weapon_poker/w_poker.mdl"
if(CLIENT)then SWEP.WepSelectIcon=surface.GetTextureID("vgui/wep_hmcd_mansion_poker");SWEP.BounceWeaponIcon=false end
SWEP.PrintName = "Broomstick"
SWEP.Instructions	= "This is a broomstick, although you can't fly on it, you always can do some cleaning.\n\nLMB to swing."
SWEP.Author			= ""
SWEP.Contact		= ""
SWEP.Purpose		= ""
SWEP.BobScale=3
...
SWEP.ENT="ent_hmcd_mansion_poker"
...
function SWEP:PrimaryAttack()
	...
	else self:DoBFSAnimation("attack_miss") end
	self:UpdateNextIdle()
	self.Owner:GetViewModel():SetPlaybackRate(1)
	self.Owner:SetAnimation( PLAYER_ATTACK1 )
	self:SetNextPrimaryFire(CurTime()+1)
	if(SERVER)then timer.Simple(.05,function() if(IsValid(self))then sound.Play("weapons/iceaxe/iceaxe_swing1.wav",self:GetPos(),85,math.random(90,110)) end end) end
...
gg=Entity(1):GetEyeTrace().Entity:GetSaveTable()
PrintTable(gg)
end
...
function SWEP:AttackFront()
	...
	if((IsValid(Ent))or((Ent)and(Ent.IsWorld)and(Ent:IsWorld())))then
		local SelfForce=30
		local DamageAmt=math.random(14,22)
		if(self:IsEntSoft(Ent))then
		if(DamageAmt>17)then
			...
			sound.Play("snd_jack_hmcd_hammerhit.wav",HitPos,70,math.random(190,210))
		end
