...
local abs = math.abs
	...
	function inflexible.hud:DrawText(text,font,x,y,col,xa,ya)
		local font_def,font_blur = !font:find('bk') and inflexible.hud:GetFont(font) or font
		xa = xa or 0
		ya = ya or 0
...
function PANEL:Paint(w, h)
	...
	if (self.HasMovedSlightly) then
		...
		if (self.selectedTbl) then
			inflexible.hud:DrawText(self.selectedTbl.name, "bkFontMedium", w / 2, h / 2, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM)
	...
	for i, v in ipairs(self.SectionsTbl) do
		...
		surface.SetDrawColor(Color(221, 235, 255))
