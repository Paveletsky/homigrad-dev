...
local PUNCH_DAMPING = 50
local PUNCH_SPRING_CONSTANT = 800
...
local function CLViewPunchThink()
    ...
    if !self.ViewPunchDone and lensqr(vpa) + lensqr(vpv) > 0.000001 then
        ...
        vpa[1] = math.Clamp(vpa[1], -209.9, 209.9)
        vpa[2] = math.Clamp(vpa[2], -179.9, 179.9)
        vpa[3] = math.Clamp(vpa[3], -45.9, 45.9)
...
local function CLViewPunchCalc(ply, pos, ang)
	...
	PunchPos:Approach(0, 0, 0, FrameTime()*233.5)
	runfwd = math.Approach(runfwd, 1, FrameTime()*45)
