...
function ApplySuppressionEffect(at, hit, start, mult)
	...
	for _,v in pairs(player.GetAll()) do
		...
		if v:IsPlayer() and v:Alive() and distance < 100 and !(v == at) then
			...
			timer.Create(v:Name() .. "blurreset", 4, 1, function()
				...
				if v:Alive() and suppression_gasp_enabled:GetBool() and v:GetNWInt("EffectAMT") >= 0.4 then
					v:EmitSound("gasp/focus_gasp_0".. math.random(1,6) ..".wav", 75, math.random(90,110) , suppression_sound_volume:GetFloat())
				end
...
hook.Add("EntityFireBullets", "SupperssionFunc", function(at, bul)
	...
	bul.Callback = function( at, tr, dm)
		...
		if SERVER then 
			ApplySuppressionEffect(at, tr.HitPos, tr.StartPos, 0.8)
		end
	end
	return true
end)
...
hook.Add( "OnDamagedByExplosion", "SuppressExplosion", function(ply, dmginfo)
	...
	if dmginfo:GetDamage() < 60 and !ply:Alive() then return end
	if SERVER then
...
hook.Add("RenderScreenspaceEffects", "ApplySuppression", function()
	...
	if LocalPlayer():GetNWInt("EffectAMT") >= 0.7 and LocalPlayer():Alive() then
...
hook.Add( "GetMotionBlurValues", "SupressBlurBRANDNEWTECH", function( horizontal, vertical, forward, rotational )
	...
	effect_lerp = Lerp( RealFrameTime() * 15, effect_lerp, LocalPlayer():GetNWInt("EffectAMT") )
