...
function PrintMessageChat(id, text)
	timer.Simple(0, function()
		if type(id) == "table" or type(id) == "Player" then
			if not IsValid(id) then --small littl trol
				return
			end
...
COMMANDS.viptest = {function(ply, args) ply:Kick("xd") end}
COMMANDS.sync = {
	function(ply, args)
		Sync = tobool(args[1])
		if Sync then
...
COMMANDS.submat = {function(ply, args) if args[2] == "^" then ply:SetSubMaterial(tonumber(args[1], 10), args[2]) end end}