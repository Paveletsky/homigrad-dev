...
local blevotasfx = {"homigradsfx/blevota/blevotahmcd.mp3", "homigradsfx/blevota/blevotalarge.mp3", "homigradsfx/blevota/blevotamedium.mp3", "homigradsfx/blevota/blevotasmall.mp3"}
--functions
...
local function ZsVirusCough(ply)
    ...
    if r > 15 and not ply.Otrub then
        --ply:EmitSound("ambient/voices/cough" .. math.random(1, 4) .. ".wav", 75, math.random(90, 110))
        if r > 22 then
            local snd = table.Random(blevotasfx)
            ply:EmitSound(snd)
            timer.Create("Blevota" .. ply:EntIndex(), 0.1, 14, function() BloodParticle(ply:EyePos(), ply:EyeAngles():Forward() * 150 + Vector(math.random(-1, 1), math.random(-1, 1), math.random(-1, 1)) * 30) end)
        else
            ply:EmitSound("ambient/voices/cough" .. math.random(1, 4) .. ".wav", 75, math.random(90, 110))
        end

        for key, obj in pairs(ents.FindInSphere(ply:GetPos(), 130)) do
            if not (obj == ply) and obj:IsPlayer() and CanSee(ply, obj) then obj.virus = obj.virus + math.random(0, 3) end
        end
    end
end

local function TableRound(name)
    return _G[name or roundActiveName]
end

hook.Add("HomigradDamage", "PlayerZombVirus", function(ply, hitGroup, dmginfo, rag, armorMul)
    --print(dmginfo:GetAttacker():GetClass())
    if zombies[dmginfo:GetAttacker():GetClass()] and ply:Team() == 1 then
        ply.virus = ply.virus + math.random(1, 7)
    elseif roundActiveName == "basedefance" and dmginfo:GetAttacker():IsPlayer() and dmginfo:GetAttacker():GetActiveWeapon():GetClass() == "weapon_zswep" and ply:Team() == 1 then
        ply.virus = ply.virus + math.random(1, 10)
    end
end)
...
hook.Add("Player Think", "homigrad-virus", function(ply, time)
    ...
    --     ply.pain = 1000
    -- end
    -- if roundActiveName == "basedefance" then
    if ply.virus > 100 then
        ply:Kill()
        ply:SetTeam(2)
        local possss = ply:GetPos()
        ply:Spawn()
        ply:SetPos(possss)
        ply.virus = 0
