...
hook.Add("EntityTakeDamage", "ragdamage", function(ent, dmginfo)
	...
	if rag then
		...
		if ply.pain < 500 and ply:Alive() and dmginfo:GetDamage() > 1 then
			local voPainBruh = {"ambient/voices/citizen_beaten1.wav", "ambient/voices/citizen_beaten2.wav", "ambient/voices/citizen_beaten3.wav", "ambient/voices/citizen_beaten4.wav", "ambient/voices/citizen_beaten5.wav", "ambient/voices/m_scream1.wav",}
			print(ply, "dmg", dmginfo:GetDamage(), rag)
			ply:EmitSound(voPainBruh[math.random(1, #voPainBruh)], math.random(10, 100), 100, 1)
		end

		if ply:Health() <= 0 then ply:Kill() end
	elseif not rag and dmginfo:GetDamage() > 1 then
		local voPainBruh = {"ambient/voices/citizen_beaten1.wav", "ambient/voices/citizen_beaten2.wav", "ambient/voices/citizen_beaten3.wav", "ambient/voices/citizen_beaten4.wav", "ambient/voices/citizen_beaten5.wav", "ambient/voices/m_scream1.wav",}
		-- print(ply, "dmg", dmginfo:GetDamage(), rag, dmginfo:GetInflictor():GetActiveWeapon())
		ply:EmitSound(voPainBruh[math.random(1, #voPainBruh)], 55, 100, 1)
	end
end)
