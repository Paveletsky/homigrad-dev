...
function Faking(ply, force) -- функция падения
	...
	if not ply.fake then
		...
		if IsValid(ply:GetNWEntity("Ragdoll")) then
			...
			if IsValid(wep) and table.HasValue(Guns, wep:GetClass()) then SpawnWeapon(ply) end
...
-- hook.Remove("Think","FakedShoot",function() --функция стрельбы лежа
...
-- end
-- end)
hook.Add("Player Think", "FakedShoot", function(ply)
...
hook.Add("PostPlayerDeath", "fuckyou", function(ply) end)
hook.Add("PhysgunDrop", "DropPlayer", function(ply, ent) ent.isheld = false end)
...
hook.Add("PreCleanupMap", "cleannoobs", function()
...
function PlayerMeta:CreateRagdoll(attacker, dmginfo, force) --изменение функции регдолла
	...
	if not self:Alive() then
		...
		net.WriteString(self.curweapon or nil)
		net.Broadcast()
	end

	rag:CallOnRemove("RemoveMoment", function() self.firstrag = false end)
	rag:CallOnRemove("RemoveMoment2", function()
...
hook.Add("Player Collide", "homigrad-fake", function(ply, hitEnt, data)
	...
	if (gg:GetBool() and not ply:HasGodMode() and data.Speed > 200) or (not gg:GetBool() and not ply:HasGodMode() and data.Speed >= 250 / hitEnt:GetPhysicsObject():GetMass() * 20 and not ply.fake and not hitEnt:IsPlayerHolding() and hitEnt:GetVelocity():Length() > 150) then
		timer.Simple(0, function()
	...
	if ply:Alive() then
		if not ply.Otrub then
			...
			if ply:KeyDown(IN_ATTACK) then
				...
				pos[3] = head:GetPos()[3]
				if not ply.FakeShooting and ply.Organs["artery"] ~= 0 then
					local phys = rag:GetPhysicsObjectNum(rag:TranslateBoneToPhysBone(rag:LookupBone("ValveBiped.Bip01_L_Hand")))
					local ang = ply:EyeAngles()
					ang:RotateAroundAxis(eyeangs:Forward(), 150)
					ang:RotateAroundAxis(eyeangs:Right(), 175)
					local shadowparams = {
						secondstoarrive = 0.2,
						pos = head:GetPos() + eyeangs:Forward() * 35 + eyeangs:Right() * -5,
						angle = ang,
						maxangular = 670,
						maxangulardamp = 600,
						maxspeeddamp = 60,
			...
			if IsValid(ply.curweapon) and ply.curweapon and weapons.Get(ply.curweapon).Primary.Automatic or false then
			...
			if ply:KeyDown(IN_ATTACK2) then
				...
				local shadowparams = {
					secondstoarrive = 0.2,
					pos = head:GetPos() + eyeangs:Forward() * 35 + eyeangs:Right() * 15,
					angle = ang,
					maxangular = 670,
					maxangulardamp = 120,
					maxspeeddamp = 50,
			...
			if ply:KeyDown(IN_USE) then
				...
				local shadowparams = {
					secondstoarrive = 0.3,
					pos = head:GetPos() + vector_up * (20 / math.Clamp(rag:GetVelocity():Length() / 200, 1, 12)),
					angle = angs,
					maxangulardamp = 10,
					maxspeeddamp = 10,
					maxangular = 600,
			...
			if not IsValid(rag.ZacConsLH) and (not rag.ZacNextGrLH or rag.ZacNextGrLH <= CurTime()) then
				...
				for i = 1, 3 do
					...
					if trace.Hit and not trace.HitSky then
						...
						if IsValid(cons) then rag.ZacConsLH = cons end
						break
					end
				end
			end
		...
		if ply:KeyDown(IN_WALK) and not RagdollOwner(rag).Otrub and not timer.Exists("StunTime" .. ply:EntIndex()) then
			...
			if not IsValid(rag.ZacConsRH) and (not rag.ZacNextGrRH or rag.ZacNextGrRH <= CurTime()) then
				...
				for i = 1, 3 do
					...
					if trace.Hit and not trace.HitSky then
						local cons = constraint.Weld(rag, trace.Entity, bone, trace.PhysicsBone, 0, false, false)
						if IsValid(cons) then rag.ZacConsRH = cons end
						break
					end
				end
			end
...
hook.Add("UpdateAnimation", "huy", function(ply, event, data) ply:RemoveGesture(ACT_GMOD_NOCLIP_LAYER) end)
hook.Remove("Player Think", "holdentity", function(ply, time)
