hook.Add("HomigradDamage", "Organs", function(ply, hitgroup, dmginfo, rag, armorMul, armorDur, haveHelmet)
    ...
    if not rag or (rag and not dmginfo:IsDamageType(DMG_CRUSH)) then
        ...
        if hitgroup == HITGROUP_HEAD and math.random(1, math.max(math.floor(armorDur), 1)) == 1 and dmginfo:IsDamageType(DMG_BULLET + DMG_SLASH + DMG_CLUB + DMG_GENERIC + DMG_BUCKSHOT) then
        ...
        if dmginfo:IsDamageType(DMG_BULLET + DMG_SLASH + DMG_BLAST + DMG_ENERGYBEAM + DMG_NEVERGIB + DMG_ALWAYSGIB + DMG_PLASMA + DMG_AIRBOAT + DMG_SNIPER + DMG_BUCKSHOT) then --and ent:LookupBone(bonename)==2 then
            ...
            if huy or huy2 then --ply:ChatPrint("You were hit in the artery.")
                if ply.Organs['artery'] ~= 0 and not dmginfo:IsDamageType(DMG_CLUB) then ply.Organs['artery'] = math.max(ply.Organs['artery'] - dmg, 0) end
            end
        end
        ...
        if huy or huy2 then --ply:ChatPrint("You were hit in the spine.")
            if ply.Organs['spine'] ~= 0 then
                ...
                if ply.Organs['spine'] == 0 then
                    timer.Simple(0.01, function() if not ply.fake then Faking(ply) end end)
...
hook.Add("HomigradDamage", "BurnDamage", function(ply, hitgroup, dmginfo) if dmginfo:IsDamageType(DMG_BURN) then dmginfo:ScaleDamage(math.random(4,9)) end end)