...
hook.Add("PlayerTick", "homigrad-player-thinker", function(ply, cmt)
	-- tbl = player_GetAll()
	time = CurTime()
	-- for i = 1, #tbl do
...
hook.Add("PlayerInitialSpawn", "homigrad-addcallback", function(ply) ply:AddCallback("PhysicsCollide", function(phys, data) hook_Run("Player Collide", ply, data.HitEntity, data) end) end)
...
util.AddNetworkString("info_staminamul2")
hook.Add("Player Think", "saystamina", function(ply, time)
	...
	if ply:GetMoveType() == MOVETYPE_WALK and ply:IsSprinting() and ply.speeed > 1 then ply.stamina = ply.stamina - 1 * ply.speeed / 150 end
	...
	if ply.stamina < 20 and ent:WaterLevel() == 3 then
		...
		if not ply.Otrub then ent:EmitSound("Player.DrownContinue", 40, 100, 0.6, CHAN_AUTO) end
	...
	net.WriteFloat(k)
	net.Send(ply)
	net.Start("info_staminamul2")
	net.WriteFloat(k)
	net.Send(ply)
	-- ply:SetNWFloat("info_staminamul", ply)
	ply.staminamul = k
end)
