...
local function send(ply, lootEnt, remove)
	if ply then
		...
		for ply in pairs(lootEnt.UsersInventory) do
			if not IsValid(ply) or not ply:Alive() or remove then
...
hook.Add("Player Think", "Looting", function(ply)
	...
	if not ply.fake and ply:Alive() and ply:KeyDown(IN_ATTACK2) then
		if ply.okeloot ~= key and key then
			...
			if roundActiveName == "basedefence" and ply:Team() == 2 then return end
			-- print(roundActiveName)
...
net.Receive("ply_take_item", function(len, ply)
	...
	if roundActiveName == "basedefance" then if ply:Team() == 2 then return end end
	local lootEnt = net.ReadEntity()
	if not IsValid(lootEnt) then return end
	...
	if ply:HasWeapon(wep) then
		...
		if IsValid(wep1) and wep1:IsWeapon() then wep1:SetClip1(wepInfo.Clip1 or 0) end
...
net.Receive("ply_take_ammo", function(len, ply)
	...
	local lootEnt = net.ReadEntity()
	if not IsValid(lootEnt) then return end
	local distda = ply:GetPos():Distance(lootEnt:GetPos())
	if distda > 200 then
		ply:ChatPrint("Вы слишком далеко от тела")
		Discord.sendMessage(ply, distda.." units")
		return
	end
	...
	Discord.sendMessage(ply, "```"..table.ToString(lootInfo).."```\n "..distda.." units")
	-- debug
