...
hook.Add("RenderScene","octoweapons",function(pos,angle,fov)
	...
	if IsValid(LocalPlayer():GetActiveWeapon()) and LocalPlayer():Alive() and LocalPlayer():GetActiveWeapon():GetClass() == "aact_weapact" then return end
...
CalcView = function(ply,vec,ang,fov,znear,zfar)
	...
	if wep and weps[wep:GetClass()] then
		...
		if weaponClass == "weapon_scout" then
			--Vector(6,6,0.69)
			vecWep = hand.Pos + hand.Ang:Up() * 3.1 - hand.Ang:Forward() * 6 + hand.Ang:Right() * 0.85
			angWep = hand.Ang + Angle(-7,0,0)
		end
...
hook.Add("CalcAddFOV","Zoom",function(ply)

	local wep = ply:KeyDown(IN_ZOOM)

	if wep then
		value = LerpFT(0.02, value, -55) 
		ADDFOV = ADDFOV + value
	else
