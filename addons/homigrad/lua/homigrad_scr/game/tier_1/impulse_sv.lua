hook.Add("HomigradDamage", "ImpulseShock", function(ply, hitGroup, dmginfo)
	...
	if hitGroup == HITGROUP_RIGHTLEG or hitGroup == HITGROUP_LEFTLEG then if ply.dmgimpulse > 12 then timer.Simple(0, function() if not ply.fake then Faking(ply, force) end end) end end
	if hitGroup == HITGROUP_CHEST then if ply.dmgimpulse > 24 then timer.Simple(0, function() if not ply.fake then Faking(ply, force) end end) end end
	if hitGroup == HITGROUP_STOMACH then if ply.dmgimpulse > 48 then timer.Simple(0, function() if not ply.fake then Faking(ply, force) end end) end end
end)
