...
resource.AddWorkshop("3134552956")
...
RTV_CountRoundDefault = 35
RTV_CountRoundMessage = 5

CountRoundRandom = CountRoundRandom or 0
RoundRandomDefalut = 1

function UpdateHostName(levelname)

	local vargamets = string.upper( levelname ) .. "" .. ""

	local done = string.format("OLD HOMIGRAD | Режим: %s", vargamets)

	RunConsoleCommand("hostname", done)        
end
...
function StartRound()
	...
	UpdateHostName(tostring(TableRound(roundActiveNameNext).Name)) 
	...
	hook.Run('HG:OnRoundStarted', roundActiveName, roundActiveNameNext)
	--DiscordSendMessage(text,textGmod ~= "" and textGmod)
...
function LevelRandom()
	for i, name in pairs(LevelList) do
		local func = TableRound(name) and TableRound(name).CanRoundNext or false
...
function RoundEndCheck()
	if SyncWait then
		...
		for steamID in pairs(player_connects) do count = count + 1 end
		
		SetGlobalVar("SyncWait",true)
		SetGlobalVar("SyncCount",count)

		if count > 0 then SyncWait = true return end

		if SyncWait == true then
			SyncWait = CurTime() + 10
			SetGlobalVar("SyncWait",SyncWait)
		elseif SyncWait < CurTime() then
			SyncWait = nil
		end
	end
	...
	timer.Simple(5,function()
		...
		if players <= 1 then
			...
			if not success then
				...
				--DiscordSendMessage("```" .. text .. "```",text)
				hook.Run('HG:OnRoundEnded', roundActiveNameNext)
