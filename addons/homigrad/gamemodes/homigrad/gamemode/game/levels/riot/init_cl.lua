...
function riot.HUDPaint_RoundLeft(white)
    ...
    if startRound > 0 and lply:Alive() then
        ...
        if name == "Полиция" then
            draw.DrawText( "Нейтрализуйте бунтующих людей, старайтесь не убивать", "fdShopFontBig", ScrW() / 2, ScrH() - 75, Color( 255,255,255,math.Clamp(startRound - 0.5,0,1) * 255 ), TEXT_ALIGN_CENTER )
        else
            draw.DrawText( "Сохраните свои права! Уничтожте всех тех, кто вас будет торомзить!", "fdShopFontBig", ScrW() / 2, ScrH() - 75, Color( 255,255,255,math.Clamp(startRound - 0.5,0,1) * 255 ), TEXT_ALIGN_CENTER )
        end
        return
    end
