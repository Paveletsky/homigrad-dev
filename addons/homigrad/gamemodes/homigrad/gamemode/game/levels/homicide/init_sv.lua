...
local function makeT(ply)
    ...
    AddNotificate(ply, "Вы мюрдер.")
    if #GetFriends(ply) >= 1 then timer.Simple(1, function() AddNotificate(ply, "Ваши товарищи " .. GetFriends(ply)) end) end
end

local function makeCT(ply2)
    ply2.roleCT = true
    table.insert(homicide.ct, ply2)
    if homicide.roundType == 1 then
        local wep = ply2:Give("weapon_remington870")
        wep:SetClip1(wep:GetMaxClip1())
        AddNotificate(ply2, "Вы невиновый с крупногабаритным огнестрельным оружием.")
    elseif homicide.roundType == 2 then
        local wep = ply2:Give("weapon_beretta")
        wep:SetClip1(wep:GetMaxClip1())
        AddNotificate(ply2, "Вы невиновый со скрытым огнестрельным оружием.")
    elseif homicide.roundType == 3 then
        --nihuya
    else
        --nihuya tozhe
...
function homicide.Spawns()
    ...
    for i, point in pairs(ReadDataMap("spawnpoints_ss_police")) do
        table.insert(aviable, point)
    end
    return aviable
end
...
function homicide.StartRoundSV()
    ...
    tdm.SpawnCommand(PlayersInGame(), aviable, function(ply)
        ply.roleT = false
        ply.roleCT = false
        if homicide.roundType == 4 then timer.Simple(0, function() ply:Give("weapon_deagle") end) end
        if ply.forceT then
            ply.forceT = nil
            countT = countT + 1
            makeT(ply)
    ...
    for i = 1, count do
        local ply = table.Random(players)
        table.RemoveByValue(players, ply)
        if homicide.roundType <= 2 then makeCT(ply) end
    end
...
COMMANDS.forcepolice = {
    function(ply)
        if not ply:IsAdmin() then
            RunConsoleCommand("ulx", "banid", ply:SteamID(), "10", "fuck off")
            return
        end
...
function homicide.EndRound(winner)
    ...
    if homicide.t and #homicide.t > 0 then PrintMessage(3, #homicide.t > 1 and ("Трейторами были: " .. homicide.t[1]:Name() .. ", " .. GetFriends(homicide.t[1])) or ("Трейтором был: " .. homicide.t[1]:Name())) end
end
...
function homicide.PlayerCanJoinTeam(ply, teamID)
    if ply:IsAdmin() then
        if teamID == 2 then
            ply.forceCT = nil
            ply.forceT = true
            ply:ChatPrint("ты будешь за дбгшера некст раунд")
            return false
        end
        ...
        if teamID == 3 then
            ply.forceT = nil
            ply.forceCT = true
            ply:ChatPrint("ты будешь за хомисайдера некст раунд")
            return false
        end
    else
