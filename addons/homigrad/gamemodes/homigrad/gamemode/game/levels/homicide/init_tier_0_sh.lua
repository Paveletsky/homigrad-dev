...
local red,blue = Color(200,0,10),Color(75,75,255)
local roundTypes = {"Чрезвычайное положение", "Обычный", "Убийственный режим", "Дикий Запад"}
...
function homicide.HUDPaint_RoundLeft(white2)
    ...
    if startRound > 0 and lply:Alive() then
        ...
        draw.DrawText( name or "Наблюдай", "fdShopFontBig", ScrW() / 2, ScrH() / 2, color and Color( color.r,color.g,color.b,math.Clamp(startRound - 0.5,0,1) * 255 ) or Color(143,109,109, math.Clamp(startRound - 0.5,0,1) * 255 ), TEXT_ALIGN_CENTER )
        draw.DrawText( roundTypes[roundType], "fdShopFontBig", ScrW() / 2, ScrH() / 2 + 35, Color( 55,55,155,math.Clamp(startRound - 0.5,0,1) * 255 ), TEXT_ALIGN_CENTER )

        if lply.roleT then
            draw.DrawText( "Ваша задача убить всех до прибытия полиции", "fdShopFontBig", ScrW() / 2, ScrH() - 75, Color( 155,55,55,math.Clamp(startRound - 0.5,0,1) * 255 ), TEXT_ALIGN_CENTER )
        elseif lply.roleCT then
            if homicide.roundType == 2 then 
                draw.DrawText( "У вас есть крупногабаритное оружие, постарайтесь нейтрализовать предателя", "fdShopFontBig", ScrW() / 2, ScrH() - 75, Color( 255,255,255,math.Clamp(startRound - 0.5,0,1) * 255 ), TEXT_ALIGN_CENTER )
            else
                draw.DrawText( "У вас есть скрытое огнестрельное оружие, постарайтесь нейтрализовать предателя", "fdShopFontBig", ScrW() / 2, ScrH() - 75, Color( 255,255,255,math.Clamp(startRound - 0.5,0,1) * 255 ), TEXT_ALIGN_CENTER )
            end
        else
            draw.DrawText( "Не доверяйте никому... даже тому с кем по дискорду играешь", "fdShopFontBig", ScrW() / 2, ScrH() - 75, Color( 255,255,255,math.Clamp(startRound - 0.5,0,1) * 255 ), TEXT_ALIGN_CENTER )
        end
