...
function homicide.HUDPaint_RoundLeft(white2)
    ...
    if startRound > 0 and lply:Alive() then
        ...
        draw.DrawText( name and "Вы " .. name or "Наблюдай", "HomigradFontBig", ScrW() / 2, ScrH() / 2, color and Color( color.r,color.g,color.b,math.Clamp(startRound - 0.5,0,1) * 255 ) or Color(143,109,109, math.Clamp(startRound - 0.5,0,1) * 255 ), TEXT_ALIGN_CENTER )
        draw.DrawText( name and "Хомисайд" or "Наблюдай, заебал", "HomigradFontBig", ScrW() / 2, ScrH() / 8, color and Color( color.r,color.g,color.b,math.Clamp(startRound - 0.5,0,1) * 255 ) or Color(143,109,109, math.Clamp(startRound - 0.5,0,1) * 255 ), TEXT_ALIGN_CENTER )
