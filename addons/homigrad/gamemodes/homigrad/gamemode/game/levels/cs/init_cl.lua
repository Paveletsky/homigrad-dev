...
function css.HUDPaint_RoundLeft(white)
	...
	local name, color = css.GetTeamName(lply) or team.GetName(1002)
    ...
    if startRound > 0 and lply:Alive() then
        ...
        draw.DrawText( "Ваша команда " .. name, "fdShopFontBig", ScrW() / 2, ScrH() / 2, Color( color.r,color.g,color.b,math.Clamp(startRound - 0.5,0,1) * 255 ), TEXT_ALIGN_CENTER )
        draw.DrawText( "Conter-Strike: Source", "fdShopFontBig", ScrW() / 2, ScrH() - 105, Color( 155,155,55,math.Clamp(startRound - 0.5,0,1) * 255 ), TEXT_ALIGN_CENTER )
        ...
        draw.DrawText( "Нейтрализуйте вражескую команду", "fdShopFontBig", ScrW() / 2, ScrH() - 75, Color( 255,255,255,math.Clamp(startRound - 0.5,0,1) * 255 ), TEXT_ALIGN_CENTER )
        return
    end
end