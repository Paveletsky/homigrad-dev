...
hook.Remove("Should Fake Collide","guilt",function(ply,hitEnt,data)
	if hitEnt == game.GetWorld() and not IsValid(hitEnt) then return end
	hitEnt = RagdollOwner(hitEnt)
	if not hitEnt:IsPlayer() and not hitEnt:Alive() then return end
