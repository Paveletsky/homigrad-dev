...
hook.Add("RenderScreenspaceEffects","SpecGoPro_Aga_Da",function()
	if not LocalPlayer():Alive() then
		...
		local spec = LocalPlayer():GetNWEntity("HeSpectateOn")
		...
		if spec then		
			...
			draw.Text( {
				text = "GoPro "..spec:Name():upper().." " .. spec:Health() ..'HP',
				font = "BodyCamFont",
				pos = { ScrW() - 180, 140 },
				xalign = TEXT_ALIGN_RIGHT,
			} )
...
hook.Add("HUDPaint","spectate",function()
	...
	then
		...
		if IsValid(ent) then
			...
			-- draw.SimpleText(ent:GetName(), "fdShopSemiFont", ScrW() / 2 - tw / 2, 35, TEXT_ALING_CENTER,TEXT_ALING_CENTER)
			-- tw = surface.GetTextSize("Здоровье: " .. ent:Health())
			-- draw.SimpleText("Здоровье: " .. ent:Health(), "fdShopSemiFont", ScrW() / 2 - tw / 2, 55,TEXT_ALING_CENTER,TEXT_ALING_CENTER)			
		...
		draw.SimpleText("Отключение / Включение отображение ников на ALT", "fdShopFontTooltip", 15, ScrH() - 15,showRoundInfoColor,TEXT_ALIGN_LEFT,TEXT_ALIGN_BOTTOM)
