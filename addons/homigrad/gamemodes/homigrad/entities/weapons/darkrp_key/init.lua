...
util.AddNetworkString("DoorsRefresh2")
...
local function canUse(ply,ent)
    ...
    for z, cpl in pairs(buy.cowner) do
        if ply == cpl then return true end
    end
end
...
net.Receive("darkrp door buy",function(len,ply)
    ...
    if darkrp.GetMoney(ply) < 25 then darkrp.Notify("Недостаточно средств.",NOTIFY_ERROR,5,ply) return end
...
net.Receive("darkrp door add",function(len,ply)
    ...
    table.insert(ent.buy.cowner, add)
    PrintTable(ent.buy)
    darkrp.Notify("Ты добавил " .. add:Name(),NOTIFY_GENERIC,5,ply)
    darkrp.Notify("Тебя добавил " .. ply:Name() .. " в качестве совладельца двери",NOTIFY_GENERIC,5,add)
end)
