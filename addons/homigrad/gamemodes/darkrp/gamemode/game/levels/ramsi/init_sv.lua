...
function ramsi.EndRound(winner) tdm.EndRoundMessage(winner) end

function ramsi.PlayerInitialSpawn(ply) ply:SetTeam(math.random(1,2)) end

function ramsi.PlayerSpawn(ply,teamID)
	local teamTbl = ramsi[ramsi.teamEncoder[teamID]]
...
function ramsi.PlayerCanJoinTeam(ply,teamID)
...
function ramsi.ShouldSpawnLoot()
...
function ramsi.PlayerDeath(ply,inf,att) return false end
