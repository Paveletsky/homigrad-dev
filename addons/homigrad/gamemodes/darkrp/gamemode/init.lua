...
function GM:PlayerDeath(ply,inf,att)
	...
	ply.allowGrab = false
	timer.Simple(2, function() ply.allowGrab = true end)
end
...
function GM:PlayerDeathThink(ply)
    ...
    if ply.EnableSpectate then
		...
		local spec = spec
		local hed = spec:LookupBone( "ValveBiped.Bip01_Head1" )
		local phed, pangda = spec:EyePos(), spec:EyeAngles()
		
		ply:SpectateEntity(spec)
		ply:SetPos(phed)
		ply:SetAngles(pangda)
