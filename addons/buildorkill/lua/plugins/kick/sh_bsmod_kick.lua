...
if CLIENT then
	...
	hook.Add("Think", "KickThink", KickThink)
...
function KickHit(ply)
	local Vec = (ply:GetAimVector() + VectorRand() * .3):GetNormalized()
    ...
    local damage = math.random(15,45) * 2
    ...
    if true then -- If we're in range
        ...
        if trace.Entity:IsPlayer() or string.find(trace.Entity:GetClass(),"npc") or string.find(trace.Entity:GetClass(),"prop_ragdoll") then	
			...
			elseif string.find(trace.Entity:GetClass(),"prop_ragdoll") then
			    phys:ApplyForceOffset(ply:GetAimVector():GetNormalized() * ((damage * 40 * 2) * 2), trace.HitPos)
	        end
			if true then
...
function FakeDoor(Door, attacker, amount)
		...
		ent:Spawn()
		ent:EmitSound("physics/wood/wood_furniture_break"..math.random(1,2)..".wav", 100)
		ent:SetVelocity(attacker:GetAimVector() * (amount * 260))
		ent:GetPhysicsObject():ApplyForceCenter(attacker:GetAimVector() * (amount * 260))
end
