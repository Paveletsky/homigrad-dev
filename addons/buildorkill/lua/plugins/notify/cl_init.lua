...
function bok.gui.Notify(text, time, path, fadeIn, fadeOut)
    ...
    if (bok.gui.CurrentNotify) and (bok.gui.CurrentNotify ~= text) then
    ...
    hook.Add('HUDPaint', 'bkNotify', function()
        local off_x, off_y = 15, 0

        table.foreach(enter, function(key, text)
                draw.WordBox(4, ScrW() - off_x, 30 + off_y, text, "fdShopSemiFont", Color( 2, 2, 2, alpha / 2 ), Color( 255, 255, 255, alpha ), TEXT_ALIGN_RIGHT, TEXT_ALIGN_RIGHT)
            off_y = off_y + 38
        end)
...
netstream.Hook('HG:Notify', bok.gui.Notify)