...
function ulx.voteCallback( ply, command, argv )
	if not ulx.voteInProgress then
		ULib.tsayError( ply, "Голосование уже кончилось, еблан" )
		return 
	end

	if not argv[ 1 ] or not tonumber( argv[ 1 ] ) or not ulx.voteInProgress.options[ tonumber( argv[ 1 ] ) ] then
		ULib.tsayError( ply, "Инвалид аргументы" )
		return
	end

	if ply.ulxVoted then
		ULib.tsayError( ply, "Ты уже голосовал" )
		return
	end
	...
	if ulx.voteInProgress.options[ id ] == "Yes" then
		ply:EmitSound("hmcd/yes.wav")
	else
		ply:EmitSound("hmcd/no.wav")
	end
	local str = ply:Nick() .. ": " .. ulx.voteInProgress.options[ id ]
	print(ulx.voteInProgress.options[ id ])
...
function ulx.stopVote( calling_ply )
	...
	calling_ply:EmitSound("hmcd/idinaxuy.wav")
	ulx.voteDone( true )
...
function ulx.votemap2( calling_ply, ... )
	...
	if #argv > 1 then
		...
		ulx.fancyLogAdmin( calling_ply, "#A зпустил голосование" .. string.rep( " #s", #argv ), ... )
	else
		ulx.doVote( "Change map to " .. argv[ 1 ] .. "?", { "Yes", "No" }, voteMapDone, _, _, _, argv, calling_ply )
		ulx.fancyLogAdmin( calling_ply, "#A Запустил голосование за #s", argv[ 1 ] )
	end
end
