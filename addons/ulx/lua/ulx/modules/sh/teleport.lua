...
function ulx.rebring( calling_ply, target_plys )
	local cell_size = 50 -- Constance spacing value
  ...
  if calling_ply:InVehicle() then
	...
	for i=1, #target_plys do
		...
		if ulx.getExclusive( v, calling_ply ) then
			ULib.tsayError( calling_ply, ulx.getExclusive( v, calling_ply ), true )
		elseif not v:Alive() or v.fake then
			v:Spawn()			
			table.insert( teleportable_plys, v )
		else
			table.insert( teleportable_plys, v )
	...
	for i=1, #tpGrid do
		...
		if tr.Hit then
			...
			target:SetTeam(1002)
			target:SetModel('models/player/charple.mdl')
	...
	if #affected_plys > 0 then
  		ulx.fancyLogAdmin( calling_ply, "#A переместил к себе #T", affected_plys )
	end
end
local rebring = ulx.command( CATEGORY_NAME, "ulx rebring", ulx.rebring, "!rebring" )
rebring:addParam{ type=ULib.cmds.PlayersArg, target="!^" }
rebring:defaultAccess( ULib.ACCESS_ADMIN )
rebring:help( "Перемещает игрока к тебе, выдает ему команду Spectator." )