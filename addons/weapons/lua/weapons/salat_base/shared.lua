...
hook.Add("HUDPaint","admin_hitpos",function()
	...
	if LocalPlayer():IsAdmin() then
		...
		local pos, ang = LocalPlayer():GetActiveWeapon():GetShootDir()

		local tr = util.QuickTrace(pos, ang:Forward() * 3000, LocalPlayer())
		local hit = tr.HitPos:ToScreen()

		draw.RoundedBox(2, hit.x-2, hit.y-2, 4, 4, Color(255,255,255,73))
	end
end)
...
function SWEP:GetShootDir(huh1, huh2)
	...
	return shootOrigin, shootAngles
end
...
function SWEP:FireBullet(dmg, numbul, spread)
	...
	if ply:GetNWBool("Suiciding") then
		if SERVER then
			...
			dmgInfo:SetDamageForce(shootDir * 2048)
...
function SWEP:Holster( wep )
	...
	if not ply == NULL and not ply:LookupBone("ValveBiped.Bip01_R_Forearm") then return end--;c
