...
local TFAVOX_Packs_Initialize = TFAVOX_Packs_Initialize
local TFAVOX_PrecachePacks = TFAVOX_PrecachePacks
local player_GetAll = player.GetAll
local print = print
local IsValid = IsValid
local TFAVOX_Init = TFAVOX_Init
...
local Color = Color
local draw_SimpleTextOutlined = CLIENT and draw.SimpleTextOutlined
local Angle = Angle
local gui_OpenURL = CLIENT and gui.OpenURL
...
local convars = {}
convars["sv_playermodel_selector_force"] = 0
...
if SERVER then
	...
	util_AddNetworkString("lf_playermodel_cvar_change")
	util_AddNetworkString("lf_playermodel_blacklist")
	util_AddNetworkString("lf_playermodel_voxlist")
	util_AddNetworkString("lf_playermodel_update")
	...
	cvars_AddChangeCallback("sv_playermodel_selector_debug", function() debugmode = GetConVar("sv_playermodel_selector_debug"):GetBool() end)
	...
	net_Receive("lf_playermodel_cvar_change", function(len, ply)
		if ply:IsValid() and ply:IsPlayer() then
			local cvar = net_ReadString()
	...
	if not file_Exists("lf_playermodel_selector", "DATA") then file_CreateDir("lf_playermodel_selector") end
	if file_Exists("playermodel_selector_blacklist.txt", "DATA") then -- Migrate from old version
		if not file_Exists("lf_playermodel_selector/sv_blacklist.txt", "DATA") then
			local content = file_Read("playermodel_selector_blacklist.txt", "DATA")
			file_Write("lf_playermodel_selector/sv_blacklist.txt", content)
		end

		file_Delete("playermodel_selector_blacklist.txt")
	end

	local Blacklist = {}
	if file_Exists("lf_playermodel_selector/sv_blacklist.txt", "DATA") then
		local loaded = util_JSONToTable(file_Read("lf_playermodel_selector/sv_blacklist.txt", "DATA"))
	...
	net_Receive("lf_playermodel_blacklist", function(len, ply)
		if ply:IsValid() and ply:IsPlayer() and ply:IsAdmin() then
			local mode = net_ReadInt(3)
			if mode == 1 then
				local gamemode = net_ReadString()
				if gamemode ~= "sandbox" then
					Blacklist[gamemode] = true
					file_Write("lf_playermodel_selector/sv_blacklist.txt", util_TableToJSON(Blacklist, true))
				end
			elseif mode == 2 then
			...
			net_Start("lf_playermodel_blacklist")
			net_WriteTable(Blacklist)
			net_Send(ply)
		end
	end)
	...
	local function InitVOX()
		if file_Exists("lf_playermodel_selector/sv_voxlist.txt", "DATA") then
			local loaded = util_JSONToTable(file_Read("lf_playermodel_selector/sv_voxlist.txt", "DATA"))
			if istable(loaded) then
				for k, v in pairs(loaded) do
	...
	net_Receive("lf_playermodel_voxlist", function(len, ply)
		if TFAVOX_Models and ply:IsValid() and ply:IsPlayer() and ply:IsAdmin() then
			...
			local mode = net_ReadInt(3)
			if mode == 1 then
				local k = net_ReadString()
				local v = net_ReadString()
				VOXlist[k] = v
				file_Write("lf_playermodel_selector/sv_voxlist.txt", util_TableToJSON(VOXlist, true))
				--TFAVOX_Models = { }
				tfa_reload()
			elseif mode == 2 then
				local tbl = net_ReadTable()
			...
			net_Start("lf_playermodel_voxlist")
			net_WriteTable(VOXlist)
			net_Send(ply)
		end
	end)
	...
	local function UpdatePlayerModel(ply)
		if Allowed(ply) then
			...
			local mdlpath = player_manager_TranslatePlayerModel(mdlname)
			SetMDL(ply, mdlpath)
			...
			if GetConVar("sv_playermodel_selector_flexes"):GetBool() and tobool(ply:GetInfoNum("cl_playermodel_selector_unlockflexes", 0)) then
				...
				local flexes = string_Explode(" ", flexes)
			...
			timer_Simple(0.1, function() if ply.SetupHands and isfunction(ply.SetupHands) then ply:SetupHands() end end)
			timer_Simple(0.2, function()
				local mdlhands = player_manager_TranslatePlayerHands(mdlname)
			...
			if addon_legs then hook_Run("SetModel", ply, mdlpath) end
		end
	end

	net_Receive("lf_playermodel_update", function(len, ply)
		if ply:IsValid() and ply:IsPlayer() and (ply:IsAdmin() or GetConVar("sv_playermodel_selector_instantly"):GetBool()) then
			if game_SinglePlayer() or ply:IsAdmin() then
				UpdatePlayerModel(ply)
			else
				local limit = math_Clamp(GetConVar("sv_playermodel_selector_limit"):GetInt(), 0, 900)
				local ct = CurTime()
				...
				elseif diff2 >= limit then
					...
					ply.lf_playermodel_lastcall = ct
					ply:ChatPrint("Enhanced PlayerModel Selector: Too many requests. Please wait another " .. tostring(limit - math_floor(diff2)) .. " seconds before trying again.")
					if debugmode then print("LF_PMS: Prevented " .. tostring(ply:GetName()) .. " from changing playermodel. Last try: " .. tostring(math_floor(diff1)) .. " seconds ago.") end
				end
			end
		end
	end)

	hook.Remove("PlayerSpawn", "lf_playermodel_force_hook1", function(ply)
	...
	local function ForceSetModel(ply, mdl)
		...
		elseif mdl then
			...
			if addon_legs then hook_Run("SetModel", ply, mdl) end
		end
	end
	...
	-- cvars_AddChangeCallback("sv_playermodel_selector_force", ToggleForce)
end
...
if CLIENT then
	...
	if not file_Exists("lf_playermodel_selector", "DATA") then file_CreateDir("lf_playermodel_selector") end
	if file_Exists("playermodel_selector_favorites.txt", "DATA") then -- Migrate from old version
		if not file_Exists("lf_playermodel_selector/cl_favorites.txt", "DATA") then
			local content = file_Read("playermodel_selector_favorites.txt", "DATA")
			file_Write("lf_playermodel_selector/cl_favorites.txt", content)
		end

		file_Delete("playermodel_selector_favorites.txt")
	end

	if file_Exists("lf_playermodel_selector/cl_favorites.txt", "DATA") then
		local loaded = util_JSONToTable(file_Read("lf_playermodel_selector/cl_favorites.txt", "DATA"))
		if istable(loaded) then
			for k, v in pairs(loaded) do
	...
	--end )
	hook_Add("PostGamemodeLoaded", "lf_playermodel_sboxcvars", function()
		if not ConVarExists("cl_playercolor") then CreateConVar("cl_playercolor", "0.24 0.34 0.41", {FCVAR_ARCHIVE, FCVAR_USERINFO, FCVAR_DONTRECORD}, "The value is a Vector - so between 0-1 - not between 0-255") end
		if not ConVarExists("cl_weaponcolor") then CreateConVar("cl_weaponcolor", "0.30 1.80 2.10", {FCVAR_ARCHIVE, FCVAR_USERINFO, FCVAR_DONTRECORD}, "The value is a Vector - so between 0-1 - not between 0-255") end
		if not ConVarExists("cl_playerskin") then CreateConVar("cl_playerskin", "0", {FCVAR_ARCHIVE, FCVAR_USERINFO, FCVAR_DONTRECORD}, "The skin to use, if the model has any") end
		if not ConVarExists("cl_playerbodygroups") then CreateConVar("cl_playerbodygroups", "0", {FCVAR_ARCHIVE, FCVAR_USERINFO, FCVAR_DONTRECORD}, "The bodygroups to use, if the model has any") end
		if not ConVarExists("cl_playerflexes") then CreateConVar("cl_playerflexes", "0", {FCVAR_ARCHIVE, FCVAR_USERINFO, FCVAR_DONTRECORD}, "The flexes to use, if the model has any") end
	end)

	local function KeyboardOn(pnl)
		if IsValid(Frame) and IsValid(pnl) and pnl:HasParent(Frame) then Frame:SetKeyboardInputEnabled(true) end
	end

	hook_Add("OnTextEntryGetFocus", "lf_playermodel_keyboard_on", KeyboardOn)
	local function KeyboardOff(pnl)
		if IsValid(Frame) and IsValid(pnl) and pnl:HasParent(Frame) then Frame:SetKeyboardInputEnabled(false) end
	end
	...
	concommand_Add("playermodel_apply", LoadPlayerModel)
	local function LoadFavorite(ply, cmd, args)
		...
		if istable(Favorites[name]) then
			...
			timer_Simple(0.1, LoadPlayerModel)
		else
			print("Favorite not found. Remember: The name is case-sensitive and should be put in quotation marks.")
		end
	end

	concommand_Add("playermodel_loadfav", LoadFavorite)
	function Menu.Setup()
		Frame = vgui_Create("DFrame")
		local fw, fh = math_min(ScrW() - 16, 960), math_min(ScrH() - 16, 700)
		Frame:SetSize(fw, fh)
		...
		if GetConVar("cl_playermodel_selector_bgcolor_custom"):GetBool() then
			local bgcolor = string_Explode(" ", GetConVar("cl_playercolor"):GetString())
			...
			if isnumber(bgcolor[1]) and isnumber(bgcolor[2]) and isnumber(bgcolor[3]) then
				r, g, b = math_Round(bgcolor[1] * 255), math_Round(bgcolor[2] * 255), math_Round(bgcolor[3] * 255)
			else
				timer_Simple(0.1, function() RunConsoleCommand("cl_playercolor", "0.24 0.34 0.41") end)
			end
		end

		local a = GetConVar("cl_playermodel_selector_bgcolor_trans"):GetBool() == true and 127 or 255
		Frame.Paint = function(self, w, h)
			draw_RoundedBox(10, 0, 0, w, h, Color(r, g, b, a))
			return true
		end

		Frame.lblTitle:SetTextColor(Color(0, 0, 0, 255))
		Frame.lblTitle.Paint = function(self, w, h)
			draw_SimpleTextOutlined(Frame.lblTitle:GetText(), "DermaDefaultBold", 1, 2, Color(255, 255, 255, 255), 0, 0, 1, Color(0, 0, 0, 255))
			return true
		end

		Frame.btnMinim:SetEnabled(true)
		Frame.btnMinim.DoClick = function() Frame:SetVisible(false) end
		...
		Menu.AdvButton.DoClick = function()
			gui_OpenURL("http://steamcommunity.com/sharedfiles/filedetails/?id=504945881")
		...
		ModelList.OnRowSelected = function()
			...
			timer_Simple(0.1, function() Menu.UpdateFromConvars() end)
		end

		local AllModels = player_manager_AllValidModels()
		function Menu.ModelPopulate()
			...
			local function IsInFilter(name)
				if not ModelFilter or ModelFilter == "" then
					...
					local tbl = string_Split(ModelFilter, " ")
					for _, substr in pairs(tbl) do
						if not string_match(name:lower(), string_PatternSafe(substr:lower())) then return false end
					end
					return true
				end
			end
		...
		function Menu.FavAdd(name)
			...
			file_Write("lf_playermodel_selector/cl_favorites.txt", util_TableToJSON(Favorites, true))
			Menu.FavPopulate()
		end
		...
		c:SetDark(true)
		c.OnChange = function(p, v) RunConsoleCommand(c.cvar, v == true and "1" or "0") end
		...
		c.OnChange = function(p, v)
			...
			timer_Simple(0, function() Menu.RebuildBodygroupTab() end)
		end
		...
		if LocalPlayer():IsAdmin() then
			...
			local function ChangeCVar(p, v)
				net_Start("lf_playermodel_cvar_change")
				net_WriteString(p.cvar)
				net_WriteString(v == true and "1" or "0")
				net_SendToServer()
			end
			...
			s.OnValueChanged = function(val)
				net_Start("lf_playermodel_cvar_change")
				net_WriteString(s.cvar)
				net_WriteString(tostring(math_floor(val:GetValue(1))))
				net_SendToServer()
			end
			...
			net_Receive("lf_playermodel_blacklist", function()
				local tbl = net_ReadTable()
				Blacklist:Clear()
				for k, v in pairs(tbl) do
					Blacklist:AddLine(k)
			...
			if TFAVOX_Models then
				...
				net_Receive("lf_playermodel_voxlist", function()
					local tbl = net_ReadTable()
					VOXlist:Clear()
					for k, v in pairs(tbl) do
						VOXlist:AddLine(string_StripExtension(string_gsub(k, "models/", "", 1)), string_StripExtension(string_gsub(v, "models/", "", 1)))
					end
				...
				if istable(TFAVOX_Models) then
					for k, v in pairs(TFAVOX_Models) do
						VOXinstalled:AddLine(string_StripExtension(string_gsub(k, "models/", "", 1)))
					end
				...
				b.DoClick = function()
					...
					local k = string_lower(player_manager_TranslatePlayerModel(LocalPlayer():GetInfo("cl_playermodel")))
					net_Start("lf_playermodel_voxlist")
					net_WriteInt(1, 3)
					net_WriteString(k)
					net_WriteString(v)
		...
		--t:SetHeight( 260 )
		t:SetAllowLua(true)
		t:AddFunction("url", "open", function(str) gui_OpenURL(str) end)
		...
		function Menu.MakeNiceName(str)
			...
			for _, s in pairs(string_Explode("_", str)) do
				if string_len(s) == 1 then
					table_insert(newname, string_upper(s))
					continue
				end

				table_insert(newname, string_upper(string_Left(s, 1)) .. string_Right(s, string_len(s) - 1)) -- Ugly way to capitalize first letters.
			end
		...
		function Menu.PlayPreviewAnimation(panel, playermodel)
			...
			local anims = list_Get("PlayerOptionsAnimations")
			local anim = default_animations[math_random(1, #default_animations)]
			if anims[playermodel] then
				anims = anims[playermodel]
				anim = anims[math_random(1, #anims)]
			end
		...
		function Menu.UpdateBodyGroups(pnl, val)
			if pnl.type == "bgroup" then
				mdl.Entity:SetBodygroup(pnl.typenum, math_Round(val))
				local str = string_Explode(" ", GetConVar("cl_playerbodygroups"):GetString())
				...
				str[pnl.typenum + 1] = math_Round(val)
				RunConsoleCommand("cl_playerbodygroups", table_concat(str, " "))
			elseif pnl.type == "flex" then
				mdl.Entity:SetFlexWeight(pnl.typenum, math_Round(val, 2))
				local str = string_Explode(" ", GetConVar("cl_playerflexes"):GetString())
				if #str < pnl.typenum + 1 then
					for i = 1, pnl.typenum + 1 do
						str[i] = str[i] or 0
					end
				...
				str[pnl.typenum + 1] = math_Round(val, 2)
				RunConsoleCommand("cl_playerflexes", table_concat(str, " "))
			elseif pnl.type == "skin" then
				mdl.Entity:SetSkin(math_Round(val))
				RunConsoleCommand("cl_playerskin", math_Round(val))
			end
		end
		...
		function Menu.RebuildBodygroupTab()
			...
			if nskins > 0 then
				local skins = vgui_Create("DNumSlider")
				skins:Dock(TOP)
				skins:SetText("Skin")
				skins:SetDark(true)
				skins:SetTall(50)
			...
			local groups = string_Explode(" ", GetConVar("cl_playerbodygroups"):GetString())
			for k = 0, mdl.Entity:GetNumBodyGroups() - 1 do
				if mdl.Entity:GetBodygroupCount(k) <= 1 then continue end
				local bgroup = vgui_Create("DNumSlider")
				bgroup:Dock(TOP)
			...
			if GetConVar("sv_playermodel_selector_flexes"):GetBool() and GetConVar("cl_playermodel_selector_unlockflexes"):GetBool() then
				...
				local flexes = string_Explode(" ", GetConVar("cl_playerflexes"):GetString())
				for k = 0, mdl.Entity:GetFlexNum() - 1 do
					if mdl.Entity:GetFlexNum(k) <= 1 then continue end
					local flex = vgui_Create("DNumSlider")
		...
		function Menu.UpdateFromConvars()
			...
			local modelname = player_manager_TranslatePlayerModel(model)
	...
	concommand_Add("playermodel_selector", Menu.Toggle)
	hook_Add("PostGamemodeLoaded", "lf_playermodel_desktop_hook", function()
		list_GetForEdit("DesktopWindows").PlayerEditor.init = function(icon, window)
			window:Remove()
	...
	local fav = {
		...
		"Leaf",
		"Mami",
		"Misaka Mikoto (Summer)",
		"Misaka Mikoto (Winter)",
		"Misaka Imoito (Summer)",
		"Misaka Imoito (Winter)",
		"Shirai Kuroko (Summer)",
		"Shirai Kuroko (Winter)",
		"Uiharu Kazari",
		"Saten Ruiko",
		"Tda Hatsune Miku (v2)",
		"YYB Kagamine Rin (v3)",
		"Appearance Miku (Default)",
		"Appearance Miku (Stroll)",
		"Appearance Miku (Cupid)",
		"Appearance Miku (Colorful Drop)",
		"Kizuna AI"
	}

	for k, v in pairs(fav) do
		list_Set("PlayerOptionsAnimations", v, bonus)
	end
end