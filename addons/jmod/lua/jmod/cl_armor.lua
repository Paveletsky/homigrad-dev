			...
			if MdlName == ArmorInfo.mdl and ArmorInfo.bon then
				...
				if Index then
					...
					if Pos and Ang then
						...
						if ply:GetModel() == "models/lightshoro_workshop/goofycat/boykisser_v2o.mdl" or ply:GetModel() == "models/eradium/nicoreda/kobodal.mdl" then
							local speed = math.floor(ply:GetVelocity():LengthSqr())
							local scol = speed > 600 and CurTime() * 100 or CurTime() * 25
							Colr = HSVToColor( scol % 360, 1, 1 )
							Mdl:SetMaterial("test_mesh_lightmap")
